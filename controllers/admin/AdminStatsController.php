...
class AdminStatsControllerCore extends AdminStatsTabController
{
    ...
    public static function getTotalSales($date_from = '', $date_to = '', $granularity = false, $id_hotel = false, $useTax = 0)
    {
        if ($granularity == 'day') {
            $sales = array();
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
                'SELECT LEFT(`invoice_date`, 10) AS date, ' . ($useTax ? 'SUM(total_paid_tax_incl / o.conversion_rate)' : 'SUM(total_paid_tax_excl / o.conversion_rate)') . ' As sales,
                (
                    SELECT hbd.`id_hotel`
                ...
                WHERE os.logable = 1'. (($date_from && $date_to) ? ' AND `invoice_date` BETWEEN "'.pSQL($date_from).' 00:00:00" AND "'.pSQL($date_to).' 23:59:59"' : '').' GROUP BY LEFT(`invoice_date`, 10)
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
                ...
                FROM `'._DB_PREFIX_.'orders` o
                LEFT JOIN `'._DB_PREFIX_.'order_state` os ON o.current_state = os.id_order_state
                WHERE os.logable = 1'. (($date_from && $date_to) ? ' AND `invoice_date` BETWEEN "'.pSQL($date_from).' 00:00:00" AND "'.pSQL($date_to).' 23:59:59"' : '').' GROUP BY LEFT(`invoice_date`, 7)
                HAVING 1 '.HotelBranchInformation::addHotelRestriction($id_hotel)
            );
    ...
    public function getLatestKpiValue($kpi)
    {
        ...
        switch ($kpi) {
            case 'conversion_rate':
                $daysForConversionRate = Configuration::get('PS_ORDER_KPI_DAYS_FOR_CONVERSION_RATE');

                $visitors = AdminStatsController::getVisits(true, date('Y-m-d', strtotime('-'.($daysForConversionRate + 1).' day')), date('Y-m-d', strtotime('-1 day')), false /*'day'*/);
                $orders = AdminStatsController::getOrders(date('Y-m-d', strtotime('-'.($daysForConversionRate + 1).' day')), date('Y-m-d', strtotime('-1 day')), false /*'day'*/);
            ...
            case 'average_order_value':
                $daysForAvgOrderVal = Configuration::get('PS_ORDER_KPI_DAYS_FOR_AVG_ORDER_VALUE');
                $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
                SELECT
            ...
            case 'netprofit_visit':
                $daysForProfitPerVisitor = Configuration::get('PS_ORDER_KPI_DAYS_FOR_PER_VISITOR_PROFIT');
